...
def compute_subset_craig(D, res, weights, m, preselect_factor=5):
    # 1. Compute per-voxel gradient vectors
    # g_i = 2 * w_i * r_i * D_i
    grad = D.multiply((2.0 * weights * res).reshape(-1, 1))

    # 2. Gradient norms (importance proxy)
    grad_norms = np.sqrt(grad.multiply(grad).sum(axis=1)).A1

    # 3. Preselect top-K voxels by gradient norm
    K = min(len(grad_norms), preselect_factor * m)
    candidate_idx = np.argsort(grad_norms)[-K:]

    G = grad[candidate_idx]

    # 4. Normalize gradient vectors
    G = normalize(G, axis=1)

    # 5. CRAIG-style greedy facility-location selection
    # similarity matrix (K × K)
    S = G @ G.T
    S = S.toarray() if hasattr(S, "toarray") else S

    selected = []
    coverage = np.zeros(K)

    for _ in range(m):
        # marginal gain for each candidate
        gains = np.maximum(coverage[:, None], S).sum(axis=0) - coverage.sum()
        gains[selected] = -np.inf  # prevent reselection
        best = np.argmax(gains)

        selected.append(best)
        coverage = np.maximum(coverage, S[:, best])

    selected = np.array(selected)
    subset_idx = candidate_idx[selected]

    # 6. Sampling probabilities q
    scores = grad_norms[subset_idx]
    q = scores / scores.sum()
